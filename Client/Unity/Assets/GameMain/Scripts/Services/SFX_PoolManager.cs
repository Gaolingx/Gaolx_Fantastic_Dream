    ...
    public class SFX_PoolManager : Singleton<SFX_PoolManager>
    {
        ...
        public async void InitSoundPool()
        {
            ...
            for (int i = 0; i < soundPools.Count; i++)
            {
                ...
                {
                    //实例化
                    var go = await ResSvc.MainInstance.LoadGameObjectAsync(Constants.ResourcePackgeName, soundPools[i].soundPath, Vector3.zero, Quaternion.Euler(Vector3.zero), Vector3.one, this.transform, true);
                    //设置父级点
