...
namespace GameMain.Editor.BuildPipeline
{
    public partial class BuildPipeline
    {
        ...
        private static void GenerateLinkfile(string outPath)
        {
            PathUtility.EnsureExistFileDirectory(outPath);
            ...
            foreach (var ass in localAssemblies)
            {
                ...
                {
                    SimpleLog.Log(ex.Message);
                    SimpleLog.Log(ex.StackTrace);
                }
            }
        ...
        {
            PathUtility.EnsureExistFileDirectory(fileName);
