...
namespace Pathfinding {
	...
	public class RecastGraphEditor : GraphEditor {
		...
		static void ColliderDetailField (RecastGraph graph) {
			const float LowestApproximationError = 0.5f;
			graph.colliderRasterizeDetail = EditorGUILayout.Slider(new GUIContent("Collider Detail", "Controls the detail of the generated sphere and capsule meshes. "+
				"Higher values may increase navmesh quality slightly, and lower values improve graph scanning performance."), graph.colliderRasterizeDetail, 0, 1.0f / LowestApproximationError);
		}
		...
		public override void OnInspectorGUI (NavGraph target) {
			...
			if (graph.dimensionMode == RecastGraph.DimensionMode.Dimension3D) {
				...
				if (graph.rasterizeTerrain) {
					...
					graph.terrainSampleSize = Mathf.Max(1, graph.terrainSampleSize);
					EditorGUI.indentLevel--;
				}
			...
			if (graph.rasterizeColliders || (graph.dimensionMode == RecastGraph.DimensionMode.Dimension3D && graph.rasterizeTerrain && graph.rasterizeTrees)) {
				ColliderDetailField(graph);
			}

			if (graph.rasterizeMeshes && graph.rasterizeColliders && graph.dimensionMode == RecastGraph.DimensionMode.Dimension3D) {
