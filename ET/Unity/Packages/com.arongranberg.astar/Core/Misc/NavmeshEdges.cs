...
namespace Pathfinding {
	...
	public class NavmeshEdges {
		...
		/// </summary>
		public NavmeshEdgeData GetNavmeshEdgeData (out RWLock.CombinedReadLockAsync readLock) {
			Init();
				...
				unsafe {
					...
					if (children.Count > 0) {
						if (children[0] is GridNodeBase) {
							...
							movementPlane = new NativeMovementPlane((children[0].Graph as NavmeshBase).transform.rotation);
						} else {
		...
		public struct NavmeshEdgeData {
			...
			/// </summary>
			public static NavmeshEdgeData CreateEmpty (Allocator allocator) {
				return new NavmeshEdgeData {
			...
			static void GetHierarchicalNodesInRangeRec (int node, Bounds bounds, SlabAllocator<int> connectionAllocator, [NoAlias] NativeList<int> connectionAllocations, NativeList<Bounds> nodeBounds, [NoAlias] NativeList<int> indices) {
				indices.Add(node);
				var conns = connectionAllocator.GetSpan(connectionAllocations[node]);
