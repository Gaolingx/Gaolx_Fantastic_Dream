	...
	public class HierarchicalGraph {
		...
		public void AddDirtyNode (GraphNode node) {
			if (!node.IsHierarchicalNodeDirty) {
				// We may be calling this when shutting down. In this case dirtyNodes may be null.
				if (dirtyNodes == null) return;

				// Mark the associated hierarchical node as dirty to ensure it is recalculated or removed later.
				// It's important to do this here because destroyed nodes are not stored in the dirty array.
				// And nodes that become destroyed later may get filtered out in CompactifyDirtyNodes.
		...
		public void ReserveNodeIndices (uint nodeIndexCount) {
			Memory.Realloc(ref dirtyNodes, (int)nodeIndexCount);
		}
		...
		struct JobRecalculateComponents : IJob {
			...
			public void Execute () {
				...
				var dirtyNodes = hGraph.dirtyNodes;
				CheckConnectionInvariants();

				// Mark the the hierarchical nodes as dirty
				for (int i = 0; i < hGraph.numDirtyNodes; i++) {
				...
				var d = hGraph.numDirtyNodes;
				for (int i = 0; i < d; i++) {
					dirtyNodes[i].HierarchicalNodeIndex = 0;
					dirtyNodes[i].IsHierarchicalNodeDirty = false;
				}
